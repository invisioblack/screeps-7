...
/**
 * This is a mess. Need to clean this up.
 * Author: Helam
 * @param id (from global.getId)
...
global.showStore = function(object, vertical=false, send=false) {
	if (object.store) {
		...
		Object.keys(object.store).forEach(function(type) {
			outstr += svgMineral(type, object.store[type]);
			if (send) {
				outstr += makeButton(id, type,'send', `function() { customCommand${id}${type}(\`Game.rooms.${object.room.name}.terminal.send('${type}', \${$('#amount${id}').val()}, '\${$('#toroom${id}').val()}')\`) }`, true);
			}
			if (vertical) {
				outstr += `\n`;
			} else {
