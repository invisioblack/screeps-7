...
MotivationHarvestMinerals.prototype.getDesireSpawn = function (roomName , demands)
{
	...
	let unitName = this.getDesiredSpawnUnit(roomName);
	let numHarvesters = 0;
	let numContainers = Room.getStructureIdsType(roomName , STRUCTURE_CONTAINER).length;
...
MotivationHarvestMinerals.prototype.updateNeeds = function (roomName)
{
	...
	let extractors = _.map(extractorIds , (id) =>
	{
	...
	extractors.forEach(function (s)
	{
		...
		{
			let containers = _.map(room.memory.cache.structures[STRUCTURE_CONTAINER] , (o) =>
			{
